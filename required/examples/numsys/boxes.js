var app = new PIXI.Application(800, 600, {backgroundColor : 0xffffff, resolution:1});
...
var texture = PIXI.Texture.fromImage('required/assets/numsys/boxer.png');
...
app.ticker.add(function(delta) {
    ...
    for (var i = 0; i < 5; i++) {
        ...
        if (sn[i].x < (boxer.x + boxer.width / 2) && sn[i].x > (boxer.x - boxer.width / 2) && sn[i].y > (boxer.y - boxer.height / 2)) {
            boxer.children[1].text = +boxer.children[1].text + +sn[i].children[1].text;
            sn[i].y = 40;
            sn.speed = getRandInt(2, 4);
            sn[i].children[1].text = getRandInt(1, Math.pow(2, bits) / 2);
