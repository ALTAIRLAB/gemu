        ...
        if (this.isDragging) {
            ...
            if (orientation == 'horizontal') {
                if (newPosition.x - x >= min && newPosition.x - x <= size) {
                    this.children[1].text = Math.round(Math.round((newPosition.x - x) / size * max) / step) * step + ' ' + unit;
                    this.x = newPosition.x - xsize / 2;
                }

            } else {
                if (y - newPosition.y >= 0 && y - newPosition.y <= size) {
                    this.children[1].text = Math.round(Math.round((y - newPosition.y) / size * max) / step) * step + ' ' + unit;
